...
namespace OnlibraryAPI.GraphQL.Queries
{
    ...
    { 
        public async Task<List<AuthorBase>> GetAuthors([Service] IDataAccess authorRepository)
        {
            List<AuthorBase> authors = new List<AuthorBase>(authorRepository.GetAuthors());
            return authors;
        }

        public async Task<List<Author>> GetLibrary([Service]IDataAccess authorRepository)
        {
            List<Author> authors = new List<Author>(authorRepository.GetLibrary());
            return authors;
        }

        public async Task<List<BookBase>> GetBooks([Service]IDataAccess authorRepository)
        {
            List<BookBase> collection = new List<BookBase>(authorRepository.GetBooks());
            return collection;
        }

        public async Task<List<Book>> GetCollection([Service]IDataAccess authorRepository)
        {
